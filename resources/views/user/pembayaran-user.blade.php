...
@section('container')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Iterate through subtotal elements and calculate total price
            subtotalElements.forEach(function(subtotalElement) {
                var subtotalValue = parseFloat(subtotalElement.value.replace('Rp. ', '').replace(',', ''));
                totalPrice += subtotalValue;
            });

            // Fetch voucher discount from the page or set it to 0 if not available
            var voucherDiscountElement = document.querySelector('.voucher-discount');

            // Subtract voucher discount from the total price
                        ...
                        <div class="row p-3">
                            ...
                            <div class="col-12 totals border-bottom py-4">
                                ...
                                <p>Note : Harap check voucher terlebih dahulu sebelum membayar untuk mendapatkan potongan
                                    harga</p>
                                <form action="{{ route('shopping-cart-voucher') }}" method="POST" class="row">
                                    @csrf
                            ...
                            <div class="col-12 totals pt-4">
                                ...
                                <div class="row">
                                    <div class="col-12 border-bottom pt-3">
                                        ...
                                        <div class="d-flex w-100 justify-content-between">
                                            <p>Voucher</p>
                                            <p class="ml-auto">- Rp {{ $voucher->jumlah_potongan ?? 0 }}</p>
                                            <input type="hidden" value="{{ $voucher->jumlah_potongan ?? 0 }}"
